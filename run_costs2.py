        ...
        for scn in self.scenarios:
            ...
            def get_stats(metric_key):
                if metric_key not in stats or stats[metric_key]['count'] == 0: 
                    return 0.0, 0.0, 0
                return stats[metric_key]['sum'], np.sqrt(stats[metric_key]['var_sum']), stats[metric_key]['count']
...
def run_pipeline():
    ...
    axes[0].set_ylabel('Cost (£ Billions)')
    ...
    # Helper to print billions in console as well
    df_summary['Cost_Billions'] = df_summary['Total_Cost'] / 1e9
    print(df_summary[['Scenario', 'Cost_Billions', 'Total_Savings', 'Metric_Mean']].round(2))
