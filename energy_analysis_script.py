...
def load_data(input_pattern):
    """Load and concatenate CSV files matching the pattern - MEMORY EFFICIENT."""
    ...
    chunk_size = 10  # Adjust based on file sizes
    dfs = []
    
    for i in range(0, len(files), chunk_size):
        chunk_files = files[i:i+chunk_size]
        print(f"Loading chunk {i//chunk_size + 1}/{(len(files)-1)//chunk_size + 1}")
...
def process_single_scenario(df, scenario_name, measure_type, years, n_simulations,
    ...
    """Process a single scenario and generate all outputs - MEMORY EFFICIENT."""
    print("\n" + "="*80)
    ...
    # Process data - MODIFY df in place, don't create copy
    ...
    # Save processed data LAST and then DELETE IT
    ...
    # CRITICAL: Clean up processed data before returning
    del df_processed
    gc.collect()
    
    print(f"\n{scenario_name} ANALYSIS COMPLETE!")
    
    return None  # Don't return the dataframe!


def main():
    # ... [keep argparse setup the same] ...
