...
def plot_building_counts_by_age_band(
    ...
    for idx, age_band in enumerate(age_bands):
        ...
        ax.set_xticks(list(counts['decile']))
        ax.set_xticklabels([str(int(x)) for x in counts['decile']])
...
def plot_building_counts_by_conservation_area(
    ...
    # Plot for Conservation Area = False (left bars) - FULL COLOR
    if False in counts['conservation_area'].values:
        ...
        p1 = ax.bar(x - width/2, cavity_false, width, 
                   label='Cavity (Non-CA)', color=colors['cavity'], alpha=1.0, 
                   edgecolor='black', linewidth=1.5)
        p2 = ax.bar(x - width/2, solid_internal_false, width, bottom=cavity_false,
                   label='Solid Internal (Non-CA)', color=colors['solid_internal'], alpha=1.0,
                   edgecolor='black', linewidth=1.5)
        p3 = ax.bar(x - width/2, solid_external_false, width, 
                   bottom=cavity_false + solid_internal_false,
                   label='Solid External (Non-CA)', color=colors['solid_external'], alpha=1.0,
                   edgecolor='black', linewidth=1.5)
    ...
    # Plot for Conservation Area = True (right bars) - LIGHTER COLOR
    if True in counts['conservation_area'].values:
        ...
        p4 = ax.bar(x + width/2, cavity_true, width, 
                   label='Cavity (CA)', color=colors['cavity'], alpha=0.7, edgecolor='black', linewidth=1.0)
        p5 = ax.bar(x + width/2, solid_internal_true, width, bottom=cavity_true,
                   label='Solid Internal (CA)', color=colors['solid_internal'], alpha=0.7, edgecolor='black', linewidth=1.0)
        p6 = ax.bar(x + width/2, solid_external_true, width, 
                   bottom=cavity_true + solid_internal_true,
                   label='Solid External (CA)', color=colors['solid_external'], alpha=0.7, edgecolor='black', linewidth=1.0)
